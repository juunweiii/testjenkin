...
RUN rm -r frontend
RUN rm -r $(ls -d backend/src/services/* | grep -v auth)
...
RUN pnpm dotenvx set PORT $(pnpm dotenvx get AUTH_SVC_PORT)
RUN pnpm dotenvx set SVC_NAME $(pnpm dotenvx get AUTH_SVC_NAME)
