...
RUN rm -r frontend
RUN rm -r $(ls -d backend/src/services/* | grep -v subtopics)
...
RUN pnpm dotenvx set PORT $(pnpm dotenvx get SUBTOPICS_SVC_PORT)
RUN pnpm dotenvx set SVC_NAME $(pnpm dotenvx get SUBTOPICS_SVC_NAME)
