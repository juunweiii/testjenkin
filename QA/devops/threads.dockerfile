...
RUN rm -r frontend
RUN rm -r $(ls -d backend/src/services/* | grep -v threads)
...
RUN pnpm dotenvx set PORT $(pnpm dotenvx get THREADS_SVC_PORT)
RUN pnpm dotenvx set SVC_NAME $(pnpm dotenvx get THREADS_SVC_NAME)
