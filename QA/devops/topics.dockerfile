...
RUN rm -r frontend
RUN rm -r $(ls -d backend/src/services/* | grep -v topics)
...
RUN pnpm dotenvx set PORT $(pnpm dotenvx get TOPICS_SVC_PORT)
RUN pnpm dotenvx set SVC_NAME $(pnpm dotenvx get TOPICS_SVC_NAME)
